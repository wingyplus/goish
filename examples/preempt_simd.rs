...
// x86_64 only: the probes load every XMM/YMM/ZMM and x87 register by
// hand and check them after a forced SIGURG, against the XSAVE layout
// the amd64 trampoline saves. arm64's trampoline has its own register
// contract (`sched_fp_callee_saved` covers the switch side), so on
// other targets this skips, like a Go test behind `//go:build amd64`.
...
fn main() {
    ...
    {
        const SKIP: &[u8] = b"preempt_simd: SKIP (amd64 vector state)\n";
